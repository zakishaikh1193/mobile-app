...
const GradesManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingGrade) {
        await api.put(`/education/grades/${editingGrade.id}`, formData);
      } else {
        await api.post('/education/grades', formData);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/education/grades/${id}`);
      fetchGrades();
    } catch (error) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Manage educational grades and academic years</p>
              </div>
            </div>
            <AnimatedButton
              variant="primary"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">No Grades Found</h3>
              <p className="text-gray-600 mb-4">Get started by adding your first grade</p>
              <AnimatedButton
                variant="primary"
                onClick={() => {
                  setEditingGrade(null);
            ...
            filteredGrades.map((grade) => (
              ...
              >
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-gray-800">{grade.name}</h3>
                    <p className="text-sm text-gray-600">{grade.academic_year}</p>
                  </div>
                </div>
                
                <p className="text-gray-700 mb-4 line-clamp-2">{grade.description}</p>
      ...
      {showAddForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <motion.div
            ...
            className="bg-white rounded-3xl p-8 shadow-xl w-full max-w-md mx-4"
          >
